...
│   ├── 实验报告1.md    # 实验报告（Markdown）
│   └── 实验报告1.pdf   # 实验报告（PDF）
├── practice02/         # 实践案例2：工具调用
│   ├── tool_client.py      # 工具调用客户端
│   ├── tool_chat_client.py # 工具调用聊天客户端
│   ├── 实验报告2.md    # 实验报告（Markdown）
│   └── 实验报告2.pdf   # 实验报告（PDF）
├── practice03/         # 实践案例3：上下文压缩
│   ├── summarize_chat_client.py  # 聊天记录总结客户端
│   ├── 实验报告3.md    # 实验报告（Markdown）
│   └── 实验报告3.pdf   # 实验报告（PDF）
├── practice04/         # 实践案例4：RAG集成
│   ├── search_chat_client.py     # 聊天历史5W提取客户端
│   ├── anythingllm_chat_client.py # AnythingLLM文档仓库客户端
│   ├── 实验报告4.md    # 实验报告（Markdown）
│   └── 实验报告4.pdf   # 实验报告（PDF）
├── practice05/         # 实践案例5：技能系统基础
│   ├── tool_client.py  # 技能管理客户端
│   ├── 实验报告5.md    # 实验报告（Markdown）
│   └── 实验报告5.pdf   # 实验报告（PDF）
├── practice06/         # 实践案例6：技能加载器
│   ├── .agents/        # 技能目录
│   │   └── skills/     # 技能存储目录
│   │       ├── notice/   # 通知撰写技能
│   │       ├── skill1/   # 自定义技能1
...
│   ├── test_notice_skill.py    # 通知技能测试
│   ├── test_notice_simple.py   # 通知简单测试
│   ├── test_integration.py     # 集成测试
│   ├── 使用说明.md      # 使用说明文档
│   ├── 实验报告6.md    # 实验报告（Markdown）
│   └── 实验报告6.pdf   # 实验报告（PDF）
├── practice07/         # 实践案例7：链式工具调用
│   ├── .agents/        # 技能目录
│   │   └── skills/     # 技能存储目录
│   │       ├── notice/   # 通知撰写技能
│   │       ├── skill1/   # 自定义技能1
│   │       └── skill2/   # 自定义技能2
│   ├── tool_client.py          # 链式工具调用客户端
│   ├── test_chained_calls.py   # 链式调用测试脚本
│   ├── summary.txt     # 总结文件
│   └── 实验报告7.md    # 实验报告（Markdown）
├── reading-notes/      # 读书心得功能目录
│   └── .agents/        # 技能目录
│       └── skills/      # 技能存储目录
│           └── init-article/  # 读书心得撰写技能
│               ├── SKILL.md          # 技能主文件
│               └── assets/           # 资源目录
│                   ├── topic.md      # 主题锚定规范
│                   ├── voice.md      # 语气风格规范
...
- **通知撰写**：撰写、修改、润色各类通知文件，包括放假通知、会议通知、公告等

**通知撰写技能使用说明：**
- 通知不能以"通知"二字开头，必须冠以部门名称前缀
...
- **场景4（读书心得撰写）**：用户请求"用init-article技能撰写一篇《边城》读书心得"，系统会调用list_available_skills→load_skill_content→读取topic.md/voice.md/structure.md/check.md→生成符合规范的读书心得
...
### 12. reading-notes/ 读书心得功能

**功能用途：**
- 提供《边城》读书心得的撰写规范和模板
- 指导大二软件工程专业学生撰写3000字以上的读书心得
- 将文学赏析与软件工程专业思维相结合，培养跨学科思考能力
...
- 技能描述和完整的撰写指南
- 针对软件工程专业学生的特色设计
- 包含数据结构视角、算法思维视角、软件工程原则等专业结合维度

#### 12.2 资源配置文件

| 文件名称 | 功能描述 |
|----------|----------|
| topic.md | 主题锚定规范，定义核心命题和边界范围 |
| voice.md | 语气与文字风格规范，设定语言参数和禁止事项 |
| structure.md | 文档结构与提纲规范，提供章节模板 |
| check.md | 文档质量检查规范，包含三级检查体系 |
...
### 13. reading-note/ 独立项目目录

**功能用途：**
- 新建的独立项目目录，用于存放读书笔记相关的独立项目
- 与主项目目录同级，便于独立管理和部署
...
**功能用途：**
- 存放课程项目报告
- 包含完整的项目报告文档（report.md）

**项目报告结构：**
- 摘要：项目概述、方法、结果、结论
- 引言：背景、优势、方法概述
- 文献综述：市场调研、参考文献
- 项目实施过程：规划、编写、开发、测试阶段
- 测试验证：测试方案、数据收集、数据分析
- 结论：研究结论、不足之处、未来方向
...
- 完整的课程讲义文档
- 包含8个单元、19课的课程内容
- 涵盖提示工程、本地LLM部署、Function Call、RAG、Skill系统、链式调用、SPEC开发范式等核心内容
...
**内容特色：**
- 约5200字，符合3000字以上要求
- 以"和谐共生"为主线，探讨茶峒社会与软件工程的关联
- 包含专业术语解释（RBAC、Raft、OAuth、高内聚低耦合等）
- 提供可量化的行动转化计划
- 涉及国家数字化建设需求的对接
...
**通知撰写技能使用示例：**
- **场景1（用户未提供部门）**：用户请求"请帮我撰写一份五一节放假通知"，系统会调用通知撰写技能，输出以"XX部通知"开头的通知
- **场景2（用户提供部门）**：用户请求"我是销售部的，请帮我撰写一份五一节放假通知"，系统会调用通知撰写技能，输出以"销售部通知"开头的通知
- **格式要求**：通知标题必须冠以部门名称前缀，不能以"通知"二字开头
...
- **链式调用**：支持多步骤工具调用，前一个工具的输出可作为后一个工具的输入
- **读书心得撰写**：提供完整的读书心得撰写规范和模板，支持跨学科内容生成
- **专业融合**：支持文学分析与专业知识的有机结合，培养跨学科思维能力
- **质量检查**：提供三级检查体系，确保输出文档的完整性、一致性和有效性
